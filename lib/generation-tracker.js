...
class GenerationTracker {
  ...
  async parseEvents(eventsFilePath) {
    if (!fs.existsSync(eventsFilePath)) {
      return { available: false, builtEpics: [], reviewFindings: [], executionTiming: {}, timeoutCount: 0, malformedLineCount: 0, testCount: null };
    }
    return new Promise((resolve) => {
      ...
      rl.on('close', () => {
        resolve({ available: true, builtEpics: [...builtEpics], reviewFindings, executionTiming, timeoutCount, malformedLineCount, testCount });
      });
    });
  }
