 ...
 * @property {number|null} timeoutWastePercentage
 */
...
class GenerationTracker {
  ...
  async parseEvents(eventsFilePath) {
    if (!fs.existsSync(eventsFilePath)) {
      return { available: false, builtEpics: [], reviewFindings: [], executionTiming: {}, timeoutCount: 0, malformedLineCount: 0, testCount: null, totalTimeoutDuration: 0 };
    }
    return new Promise((resolve) => {
      ...
      rl.on('line', (line) => {
        ...
        if (event.type === 'error' && event.message && /timeout|timed.out|SIGTERM/i.test(event.message)) {
          timeoutCount++;
          if (event.duration) {
            totalTimeoutDuration += event.duration;
          } else if (event.layer) {
            const key = `${event.layer}:${event.epic || 'all'}`;
            if (executionTiming[key] && executionTiming[key].duration) {
              totalTimeoutDuration += executionTiming[key].duration;
            }
          }
        }
        if (event.type === 'test_results' && event.count != null) {
          testCount = event.count;
        }
        if (event.type === 'layer_end' && event.meta && event.meta.numTotalTests != null) {
          testCount = event.meta.numTotalTests;
        }
        if (event.type === 'layer_end' && event.meta && event.meta.testCount != null) {
          testCount = event.meta.testCount;
        }
      });

      rl.on('close', () => {
        resolve({ available: true, builtEpics: [...builtEpics], reviewFindings, executionTiming, timeoutCount, malformedLineCount, testCount, totalTimeoutDuration });
      });
    });
  }
  ...
  async analyze() {
    ...
    for (const gen of discovered) {
      ...
      records.push({
        ...
        testCount: eventData.testCount ?? null,
        timeoutWastePercentage: (executionTime && executionTime > 0 && eventData.totalTimeoutDuration != null)
          ? (eventData.totalTimeoutDuration / executionTime) * 100
          : null,
      });
    }
