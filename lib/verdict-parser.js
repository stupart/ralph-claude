...
const VERDICT_PATTERNS = [
  ...
  /^(PASS|ITERATE)\s*$/m,
  // Expanded patterns: bold markdown emphasis on standalone line
  /(?:^|\n)\*\*(PASS|ITERATE)\*\*\s*(?:\n|$)/i,
  // Expanded patterns: bare heading without "Verdict:" prefix (## PASS, # ITERATE)
  /(?:^|\n)#{1,3}\s+(PASS|ITERATE)\s*(?:\n|$)/i,
  // Expanded patterns: "Overall" prefix (Overall Verdict: PASS, Overall: ITERATE)
  /Overall\s*(?:Verdict)?[:\s]+(PASS|ITERATE)/i,
];
...
class VerdictParser {
  ...
  parse(output, context = {}) {
    ...
    if (verdict === 'PASS' && hasHighSeverity) {
      ...
      return { verdict: 'ITERATE', issues, rawOutput: text, parsedExplicitly: false };
    }
    ...
    if (verdict === null) {
      ...
      if (hasHighSeverity) {
        ...
        return { verdict: 'PASS', issues, rawOutput: text, parsedExplicitly: false };
      }
    }

    return { verdict, issues, rawOutput: text, parsedExplicitly: true };
  }
}
