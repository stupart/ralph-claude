...
class Ralph {
  ...
  async runLayerCycle(agentExecutor) {
    ...
    //    and proper subprocess kill enforcement.
    //
    //    The executor receives spawnConfig with an `abortSignal` property.
    //    On timeout we:
    //      a) Fire the AbortController to signal the executor
    //      b) Kill any child process the executor registered via spawnConfig.registerProcess()
    //      c) Reject with a timeout error
    let artifacts;
    ...
    if (timeout && timeout > 0) {
      ...
      try {
        ...
        this.eventLogger.error(spawnResult.layerId, err.message);
