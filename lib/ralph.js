...
const { StallDetector, DEFAULT_LAYER_THRESHOLDS } = require('./stall-detector');
const { Notifier } = require('./notifier');
const { Webhook } = require('./webhook');
const { PluginManager } = require('./plugin');
const { VerdictParser } = require('./verdict-parser');
const { summarizeL3, summarizeL7 } = require('./gate-summarizer');
const { promptGate, storeFeedback, clearFeedback } = require('./gate-prompt');

let ClaudeExecutor;
try {
  ({ ClaudeExecutor } = require('./claude-executor'));
} catch (e) {
  ClaudeExecutor = null;
}
...
/**
 * Tracks wall-clock timing per layer execution.
 * Records start/end timestamps and computes elapsed duration in ms.
 */
class LayerTimer {
  constructor() {
    /** Per-layer timing records: { [layerId]: { startedAt, endedAt, durationMs, runs: [] } } */
    this.layers = {};
    this.totalDurationMs = 0;
  }

  /**
   * Mark the start of a layer execution.
   * @param {string} layerId
   */
  start(layerId) {
    if (!this.layers[layerId]) {
      this.layers[layerId] = { startedAt: null, endedAt: null, durationMs: 0, runs: [] };
    }
    this.layers[layerId].startedAt = new Date().toISOString();
    this.layers[layerId].endedAt = null;
  }

  /**
   * Mark the end of a layer execution and record duration.
   * @param {string} layerId
   */
  end(layerId) {
    const entry = this.layers[layerId];
    if (!entry || !entry.startedAt) return;

    entry.endedAt = new Date().toISOString();
    const elapsed = new Date(entry.endedAt).getTime() - new Date(entry.startedAt).getTime();
    entry.durationMs += elapsed;
    entry.runs.push({ startedAt: entry.startedAt, endedAt: entry.endedAt, durationMs: elapsed });
    this.totalDurationMs += elapsed;
  }

  /**
   * Get a summary of all layer timings.
   * @returns {{ layers: Object, totalDurationMs: number }}
   */
  getSummary() {
    const layerSummary = {};
    for (const [layerId, data] of Object.entries(this.layers)) {
      layerSummary[layerId] = {
        durationMs: data.durationMs,
        runs: data.runs.length,
        lastStartedAt: data.startedAt,
        lastEndedAt: data.endedAt
      };
    }
    return { layers: layerSummary, totalDurationMs: this.totalDurationMs };
  }
...
class Ralph {
  constructor(projectRoot, options = {}) {
    ...
    this.options = {
      ...
      agentTimeout: 300000,
      /** Per-layer timeout overrides in ms. Keys are layer IDs. 0 = no timeout. */
      layerTimeouts: {
        L6: 1800000,  // 30 min - task specification generates many files
        L7: 1800000,  // 30 min - subtask definition generates many files
        L8: 0         // No timeout - builder runs until done (per-epic scoping keeps context manageable)
      },
      ...
      dryRun: false,
      /** Maximum cascade depth before escalating to human intervention. Default: 5. */
      maxCascadeDepth: 5,
      /** Timeout in ms for human gates. 0 means no timeout. Default: 0 (wait forever). */
      gateTimeout: 0,
      /** When true, auto-approve gates that exceed gateTimeout instead of erroring. Default: false. */
      gateAutoApproveOnTimeout: false,
      ...options
    };

    // Cascade depth tracking: counts consecutive builder/judge bounces without a PASS
    this._cascadeDepth = 0;

    // Gate waiting timestamps: { [layerId]: Date.now() }
    this._gateWaitStart = {};
    ...
    this.timings = new LayerTimer();
    this.executor = ClaudeExecutor ? new ClaudeExecutor() : null;
    this.verdictParser = new VerdictParser();

    // Previous iteration issues for builder re-prompting (iteration learning)
    this._previousIterationIssues = null;
    ...
    this.stallDetector = new StallDetector(projectRoot, {
      ...
      layerThresholds: options.layerThresholds || DEFAULT_LAYER_THRESHOLDS,
      killOnCritical: options.killOnCritical || false,
      onStall: (info) => {
        this.log(`STALL WARNING: No status update for ${Math.round(info.elapsedMs / 1000)}s (layer: ${info.tier})`);
      ...
      onCritical: (info) => {
        this.log(`STALL CRITICAL: No status update for ${Math.round(info.elapsedMs / 1000)}s (2x threshold exceeded)`);
        this.eventLogger.log({
          type: 'stall_critical',
          message: `Critical stall: no status update for ${Math.round(info.elapsedMs / 1000)}s`,
          meta: info
        });
        this.emit('onError', { type: 'stall_critical', ...info });
      }
    });
   ...
   * @returns {Ralph} this (for chaining)
   */
  use(plugin) {
  ...
  async runNextLayer() {
    ...
    await this.pluginManager.run('beforeLayerStart', { layerId, layer, position: currentState.position });
    ...
    if (layer.humanGate) {
      ...
      if (gateStatus !== 'approved') {
        // Check gate timeout
        if (this.options.gateTimeout && this.options.gateTimeout > 0) {
          if (!this._gateWaitStart[layerId]) {
            this._gateWaitStart[layerId] = Date.now();
          }

          const elapsed = Date.now() - this._gateWaitStart[layerId];
          if (elapsed >= this.options.gateTimeout) {
            const { formatError } = require('./errors');
            const action = this.options.gateAutoApproveOnTimeout
              ? 'Auto-approving gate.'
              : 'Notification sent.';
            const msg = formatError('ERR_GATE_TIMEOUT', {
              layer: layerId,
              timeoutMs: this.options.gateTimeout,
              action
            });

            this.log(msg);
            this.emit('onGateTimeout', { layerId, elapsed, autoApproved: this.options.gateAutoApproveOnTimeout });
            this.notifier.error(layerId, msg);
            this.eventLogger.log({
              type: 'gate_timeout',
              message: msg,
              meta: { layer: layerId, elapsed, autoApproved: this.options.gateAutoApproveOnTimeout }
            });

            if (this.options.gateAutoApproveOnTimeout) {
              delete this._gateWaitStart[layerId];
              await this.approveGate(layerId);
              // Re-read state after approval and continue
            ...
            this.log(`Human gate at ${layerId} - waiting for approval (${elapsed}ms/${this.options.gateTimeout}ms)`);
          ...
          this.log(`Human gate at ${layerId} - prompting for approval...`);
          this.emit('onHumanGateRequired', { layerId, layer });

          try {
            // Determine gate type and call appropriate summarizer
            let summary;
            if (layerId === 'L3') {
              summary = await summarizeL3(this.projectRoot);
            } else if (layerId === 'L7') {
              summary = await summarizeL7(this.projectRoot);
            } else {
              summary = { message: `Gate at ${layerId}` };
            }

            // Prompt user for decision (auto-approve if configured)
            const decision = await promptGate(summary, layerId, {
              autoApprove: this.options.autoApproveGates
            });

            if (decision.approved) {
              this.log(`Gate at ${layerId} approved`);

              // Clear any stale feedback when approving
              try {
                await clearFeedback(this.projectRoot, layerId);
              } catch (err) {
                this.log(`Warning: Failed to clear feedback: ${err.message}`);
              }

              await this.approveGate(layerId);
              await this.state.read();
            } else {
              this.log(`Gate at ${layerId} rejected by user`);

              if (decision.feedback) {
                this.log(`Feedback received: ${decision.feedback}`);
                await storeFeedback(this.projectRoot, layerId, decision.feedback);
              }

              this.emit('onGateRejection', { layerId, feedback: decision.feedback });
              this.eventLogger.log({
                type: 'gate_rejected',
                layer: layerId,
              ...
              return {
                ...
                message: `Gate at ${layerId} rejected - re-execution required`
              };
            }
          } catch (err) {
            this.log(`Gate prompt error: ${err.message}`);
            this.emit('onError', { type: 'gate_prompt_error', layerId, error: err.message });
            return {
              status: 'gate_error',
              layerId,
              error: err.message,
              message: `Gate prompt failed: ${err.message}`
            };
          }
        }
      }
    ...
    // Build handoff with iteration learning if previous issues exist
    let handoff = null;
    if (this._previousIterationIssues && this._previousIterationIssues.length > 0) {
      handoff = {
        previousIssues: this._previousIterationIssues,
        iterationNumber: currentState.position.iteration,
        maxIterations: this.options.maxRetries
      };
    }
    ...
    await this.pluginManager.run('beforeSpawn', { layerId, spawnConfig });
  ...
  async onLayerComplete(layerId, artifacts = {}) {
    ...
    // Clear any stored feedback for this layer now that it succeeded
    try {
      await clearFeedback(this.projectRoot, layerId);
    } catch (err) {
      this.log(`Warning: Failed to clear feedback for ${layerId}: ${err.message}`);
    }

    // Parse verdict from raw output for judge layers (L9/L10/L11)
    if (LAYERS[layerId] && LAYERS[layerId].agent === 'judge' && !artifacts.reviewResult) {
      const rawOutput = artifacts.rawOutput || artifacts.output || '';
      artifacts.reviewResult = this.verdictParser.parse(rawOutput);
      this.log(`[${layerId}] Verdict: ${artifacts.reviewResult.verdict}, Issues: ${artifacts.reviewResult.issues.length}`);
    }
    ...
    if (isReviewLayer && artifacts.reviewResult) {
      ...
      await this.pluginManager.run('onVerdict', { layerId, verdict, issues });
      return await this.handleReviewResult(layerId, artifacts.reviewResult);
    }

    // Otherwise, advance to next layer
    let routingDecision = await this.state.advance();

    // Handle human gate blocking advancement
    if (routingDecision.action === 'waiting_human') {
      if (this.options.autoApproveGates) {
        this.log(`Auto-approving gate at ${layerId} (post-completion)`);
        await this.approveGate(layerId);
        routingDecision = await this.state.advance();
      } else {
        this.log(`Human gate at ${layerId} - waiting for approval before advancing`);
        this.emit('onHumanGateRequired', { layerId });
        this.eventLogger.gateWaiting(layerId);
        return {
          status: 'waiting_human',
          layerId,
          message: `Layer ${layerId} requires human approval before advancing`
        };
      }
    }
    ...
    // Handle pipeline completion (L12→COMPLETE): state.advance() returns
    // { action: 'complete', layer: 'COMPLETE' } without a .to field
    if (routingDecision.action === 'complete') {
      this.emit('onLayerComplete', { layerId, next: 'COMPLETE' });
      this.eventLogger.layerEnd(layerId, 'COMPLETE');
      this.notifier.layerTransition(layerId, 'COMPLETE');
      this.webhook.layerEnd(layerId, 'COMPLETE');
      await this.pluginManager.run('afterLayerEnd', { layerId, next: 'COMPLETE', result: { status: 'complete', from: layerId, to: 'COMPLETE', action: 'complete' } });
      return { status: 'complete', from: layerId, to: 'COMPLETE', action: 'complete', message: `Pipeline complete from ${layerId}` };
    }
    ...
    await this.pluginManager.run('afterLayerEnd', { layerId, next: routingDecision.to, result });
    return result;
  }
  ...
  async handleReviewResult(layerId, reviewResult) {
    ...
    // Store issues for iteration learning when routing back to builder
    if (reviewResult.issues && reviewResult.issues.length > 0) {
      this._previousIterationIssues = reviewResult.issues;
    }
    ...
    // Clear previous issues and reset cascade depth when advancing (PASS verdict)
    if (routingDecision.action === 'advance') {
      this._previousIterationIssues = null;
      this._cascadeDepth = 0;
    }

    // Track cascade depth for iterate/cascade actions (builder/judge bouncing)
    if (routingDecision.action === 'cascade' || routingDecision.action === 'iterate') {
      this._cascadeDepth++;
      this.log(`Cascade depth: ${this._cascadeDepth}/${this.options.maxCascadeDepth}`);

      if (this._cascadeDepth >= this.options.maxCascadeDepth) {
        const { formatError } = require('./errors');
        const msg = formatError('ERR_CASCADE_DEPTH', {
          maxDepth: this.options.maxCascadeDepth,
          layer: layerId,
          depth: this._cascadeDepth
        });
        this.log(msg);
        this.eventLogger.log({
          type: 'cascade_depth_exceeded',
          message: msg,
          meta: { layer: layerId, depth: this._cascadeDepth, maxDepth: this.options.maxCascadeDepth }
        });
        this.notifier.error(layerId, msg);
        this.emit('onError', { type: 'cascade_depth_exceeded', layer: layerId, depth: this._cascadeDepth });
        return {
          status: 'human_required',
          action: 'human_required',
          layer: layerId,
          reason: msg,
          cascadeDepth: this._cascadeDepth
        };
      }
    }
  ...
  async getStatus() {
    ...
    return {
      ...
      timings: this.timings.getSummary()
    };
  }
   ...
   * @param {Object} spawnConfig.toolPermissions - Tool permissions for the agent.
  ...
  async runLayerCycle(agentExecutor) {
    ...
    // Update stall detector with current layer for per-layer thresholds
    this.stallDetector?.setLayer(spawnResult.layerId);
    ...
    // Start wall-clock timing for this layer
    this.timings.start(spawnResult.layerId);
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt === maxRetries) {
          // All retries exhausted — check for partial completion
          // If the agent timed out but produced valid artifacts, auto-advance.
          // Only check layers with real validation rules (L3-L7); other layers
          // have no artifact checks so a passed result would be meaningless.
          const isTimeout = err.message.includes('timed out');
          const hasValidation = ['L3', 'L4', 'L5', 'L6', 'L7'].includes(spawnResult.layerId);
          if (isTimeout && hasValidation) {
            this.log(`All retries exhausted with timeout at ${spawnResult.layerId}, checking for partial completion...`);
            try {
              const validationResult = await this.validator.validateLayer(spawnResult.layerId);
              if (validationResult.passed) {
                this.log(`Partial completion detected at ${spawnResult.layerId}: validation passed despite timeout`);
                this.eventLogger.log({
                  type: 'partial_completion',
                  message: `Agent timed out but validation passed at ${spawnResult.layerId}`,
                  meta: { layerId: spawnResult.layerId, attempts: attempt + 1 }
                });
                this.timings.end(spawnResult.layerId);
                // Treat as success — proceed to onLayerComplete
                artifacts = { output: '', partialCompletion: true };
                lastError = null;
                break;
              }
              this.log(`Partial completion check failed: validation did not pass`);
            } catch (valErr) {
              this.log(`Partial completion check error: ${valErr.message}`);
            }
          }

          // No partial completion — fail as before
          const errorInfo = {
            type: 'agent_error',
            layerId: spawnResult.layerId,
            error: err.message,
          ...
          this.emit('onError', errorInfo);
          this.timings.end(spawnResult.layerId); // Stop timer on error
          ...
          await this.pluginManager.run('onError', errorInfo);
          return {
            status: 'error',
            layerId: spawnResult.layerId,
            error: err.message,
    ...
    // 3. Run afterSpawn plugin hook
    await this.pluginManager.run('afterSpawn', { layerId: spawnResult.layerId, spawnConfig: spawnResult.spawnConfig, artifacts });

    // 4. Record token usage if provided by the agent executor
    ...
    // 4b. Log agent_complete event with token and verdict data
    const tokenUsage = (artifacts && artifacts.tokenUsage) || { inputTokens: 0, outputTokens: 0 };
    const agentCompleteEvent = {
      type: 'agent_complete',
      layer: spawnResult.layerId,
      agent: LAYERS[spawnResult.layerId]?.agent || 'unknown',
      tokenUsage,
      timestamp: new Date().toISOString()
    };
    if (LAYERS[spawnResult.layerId]?.agent === 'judge' && artifacts && artifacts.reviewResult) {
      agentCompleteEvent.verdict = artifacts.reviewResult.verdict;
      agentCompleteEvent.issueCount = artifacts.reviewResult.issues.length;
    }
    this.eventLogger.log(agentCompleteEvent);

    // 5. Stop wall-clock timer and log timing
    this.timings.end(spawnResult.layerId);
    const layerTiming = this.timings.layers[spawnResult.layerId];
    if (layerTiming) {
      const lastRun = layerTiming.runs[layerTiming.runs.length - 1];
      this.log(`Layer ${spawnResult.layerId} completed in ${lastRun.durationMs}ms`);
      this.eventLogger.log({
        type: 'layer_timing',
        layer: spawnResult.layerId,
        message: `Layer ${spawnResult.layerId} completed in ${lastRun.durationMs}ms`,
        meta: { durationMs: lastRun.durationMs, startedAt: lastRun.startedAt, endedAt: lastRun.endedAt }
      });
    }
  ...
  async _executeAgent(agentExecutor, spawnResult) {
    const layerId = spawnResult.layerId;
    const layerTimeouts = this.options.layerTimeouts || {};
    // Use nullish coalescing: layerTimeouts[L8] = 0 means "no timeout", not "use default"
    const timeout = layerId in layerTimeouts ? layerTimeouts[layerId] : this.options.agentTimeout;
  ...
  async runProject(agentExecutor) {
    // Default to ClaudeExecutor when no custom executor provided
    if (!agentExecutor) {
      if (!this.executor) {
        throw new Error('No agentExecutor provided and ClaudeExecutor is not available. Install lib/claude-executor.js or provide a custom executor.');
      }
      agentExecutor = (spawnConfig) => this.executor.execute(spawnConfig, {
        signal: AbortSignal.timeout(this.options.agentTimeout)
      });
    }
    ...
    do {
      ...
      if (state.position.layer === 'COMPLETE') {
        this.log('Project already complete, nothing to do');
        return { status: 'complete', message: 'Project already completed' };
      }
      ...
      if (state.position.layer === 'L8') {
        ...
        await this.state.write();
        // Fall through to runLayerCycle for L10 (don't use continue — in do-while it
        // jumps to the condition check where result is still undefined)
      }
      ...
      if (result.status === 'error') {
        this.log('Layer execution failed, stopping pipeline');
        return result;
      }

    } while (result && result.to !== 'COMPLETE' && result.status !== 'complete' && result.action !== 'complete');

    if (result.to === 'COMPLETE' || result.action === 'complete' || result.status === 'complete') {
      this.log('Project complete!');
      this.emit('pipeline_complete', { status: 'complete', finalLayer: 'L12' });
...
module.exports = { Ralph, CostTracker, LayerTimer, PluginManager };
