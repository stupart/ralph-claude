...
class Router {
  ...
  async routeMinor(currentLayer, iteration) {
    ...
    // For review layers, MINOR goes to Builder (L8)
    if (typeof layerDef?.onFail === 'object' && layerDef.onFail.minor) {
  ...
  async routeMajor(currentLayer, iteration) {
    ...
    // Get target from layer definition
    if (typeof layerDef?.onFail === 'object' && layerDef.onFail.major) {
  ...
  async routeEscalate(currentLayer, iteration) {
    ...
    // Get escalation target from layer definition
    if (typeof layerDef?.onFail === 'object' && layerDef.onFail.escalate) {
