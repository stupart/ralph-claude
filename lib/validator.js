...
class Validator {
  ...
  async validateEpics(result) {
    ...
    try {
      ...
      // Check each epic has required sections
      ...
      if (err.code === 'ENOENT') {
        result.addError('Missing epics.md file', epicsPath);
      } else {
  ...
  /**
   * Get epic count from L4 epics.md for cross-layer validation (BUG-001 fix)
   */
  ...
  countMarkdownItems(content, type) {
    ...
    switch (type) {
      ...
      case 'decisions':
      case 'decision':
        pattern = /^##\s+(Decision|ADR)\s+\d+/gmi;
        break;
      case 'epic':
  ...
  hasSection(content, sectionName) {
    const pattern = new RegExp(`^#{1,4}\\s*${sectionName}`, 'gmi');
