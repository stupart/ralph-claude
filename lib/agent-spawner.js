...
class AgentSpawner {
  ...
  async createSpawnConfig(layerId, position = {}, handoff = null) {
    ...
    // Append iteration context based on agent type
    let iterationContext = '';
    if (handoff && handoff.previousIssues && handoff.previousIssues.length > 0) {
      if (agentType === 'judge') {
        iterationContext = this.buildStructuredIterationContext(
          handoff.previousIssues,
          handoff.iterationNumber || 1,
          handoff.maxIterations
        );
      } else if (agentType === 'builder') {
        iterationContext = this.buildIterationLearning(
          handoff.previousIssues,
          handoff.iterationNumber || 1
        );
      }
      // planners and unknown types: no iteration context
    } else if (handoff && agentType === 'judge' && handoff.iterationNumber) {
      // Judge with no prior issues but valid iteration number: show counter only
      iterationContext = this.buildStructuredIterationContext(
        [],
        handoff.iterationNumber,
        handoff.maxIterations
      );
    }
  ...
  buildStructuredIterationContext(previousIssues, iterationNumber, maxIterations) {
    ...
    // Always include iteration counter when iterationNumber is provided
    if (iterationNumber) {
      parts.push('## Review Iteration');
      parts.push('');
      let counterLine = `This is iteration ${iterationNumber} of ${max} (max).`;
      if (iterationNumber === max) {
        counterLine += ' This is the FINAL iteration — focus exclusively on verifying prior findings were addressed.';
      }
      parts.push(counterLine);
    }

    // Include issues checklist if there are previous issues
    if (previousIssues && previousIssues.length > 0) {
      parts.push('');
      parts.push('## Prior Iteration Issues (Verify Fixed)');
      parts.push('');
      ...
      for (let i = 0; i < previousIssues.length; i++) {
        ...
        if (issue.description) {
          parts.push(`${i + 1}. [${severity}] ${title}: ${issue.description}`);
        } else {
          parts.push(`${i + 1}. [${severity}] ${title}`);
        }
      }
    }
