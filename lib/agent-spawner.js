...
const TOOL_PERMISSIONS = {
  planner: {
    ...
    forbidden: ['Edit', 'Bash', 'NotebookEdit']
  },
  builder: {
...
class AgentSpawner {
  ...
  getToolPermissions(agentType) {
    return TOOL_PERMISSIONS[agentType] || TOOL_PERMISSIONS.planner;
  }
  ...
  async loadPromptTemplate(agentType, layerId = null) {
    ...
    try {
      return await fs.readFile(basePath, 'utf8');
    } catch {
      // Try without -base suffix
  ...
  buildLayerInstructions(layerId, position) {
    ...
    switch (layerId) {
      ...
      case 'L8':
        ...
        break;
      case 'L9':
        lines.push('', '1. Review the implemented feature', '2. Test via /chrome for UX', '3. Verify acceptance criteria', '4. Issue PASS or ITERATE with severity');
        break;
      case 'L10':
        lines.push('', '1. Review epic integration', '2. Test cross-feature interactions', '3. Verify epic goal is met', '4. Issue PASS or ITERATE with severity');
        break;
      case 'L11':
        lines.push('', '1. Final review of entire project', '2. Walk through all user journeys', '3. Verify ship-worthiness', '4. Issue PASS or ITERATE with severity');
        break;
      case 'L12':
  ...
  validateToolUsage(agentType, tool, args = {}) {
    const permissions = TOOL_PERMISSIONS[agentType];
    if (!permissions) {
...
module.exports = { AgentSpawner, AGENT_MODEL, AGENT_CONFIGS, LAYER_AGENTS, TOOL_PERMISSIONS, LAYER_CONTEXT };
