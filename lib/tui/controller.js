...
function handleAction(action) {
  // E3-F3-T2-S2: When in text input mode, intercept actions as text events.
  // Escape (dismiss-overlay) cancels, quit still works for safety.
  if (controllerState && controllerState.inputMode === 'text') {
    if (action === 'quit') { stop(); return; }
    if (action === 'dismiss-overlay') { cancelDenyGate(); return; }
    // All other mapped keys are absorbed in text mode
    return;
  }
...
// ─── E3-F3-T2-S2: Raw character routing for text input mode ──────────

function handleRawChar(ch) {
  if (controllerState && controllerState.inputMode === 'text') {
    handleTextInput(ch);
  }
}
...
function start(mode, options) {
  ...
  inputHandler.onRawChar(handleRawChar);
}
