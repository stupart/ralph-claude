...
function render(state, terminalSize, frameCount, controllerState) {
  ...
  if (controllerState.overlay) {
    buf += renderOverlay(controllerState.overlay, state, terminalSize);
  }

  // Show cursor when user input is expected
  const needsCursor = state.gateWaiting || controllerState.overlay;
