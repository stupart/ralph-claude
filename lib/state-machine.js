 ...
 * Maps each layer to the layers whose artifacts it requires as input.
 * Used by agent-spawner for progressive context loading and
 * recovery.js for smarter resume logic.
 *
 * Example: L4 depends on L3 (synthesis) to define epics.
 *          L8 depends on L7 (subtasks) to know what to build.
 *          L9 depends on L5 (feature specs) and L8 (built code) to review.
 */
const LAYER_DEPS = {
  L1: [],                    // No dependencies - reads raw input
  L2: ['L1'],                // Decomposes L1 input
  L3: ['L1', 'L2'],          // Synthesizes from input + decomposition
  L4: ['L3'],                // Defines epics from synthesis
  L5: ['L3', 'L4'],          // Plans features from synthesis + epics
  L6: ['L5'],                // Specifies tasks from features
  L7: ['L5', 'L6'],          // Defines subtasks from features + tasks
  L8: ['L7'],                // Builds from subtask specs
  L9: ['L5', 'L7'],          // Reviews feature against specs + subtasks
  L10: ['L4', 'L5'],         // Reviews epic integration against epic + feature specs
  L11: ['L3', 'L4'],         // Final review against synthesis + epics
  L12: []                    // Retrospective loads everything (handled separately)
};
...
module.exports = { StateManager, LAYERS, LAYER_DEPS, LAYER_FOLDERS, DEFAULT_STATE, createDefaultState, acquireLock, releaseLock };
